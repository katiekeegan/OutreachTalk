...
const capstoneDatasetStats = document.getElementById("capstoneDatasetStats");
...
function renderLiveState(state) {
  ...
  if (capstoneDatasetStats && !state.approved?.length) {
    capstoneDatasetStats.textContent = state.counts?.pending
      ? `${state.counts.pending} example${state.counts.pending === 1 ? " is" : "s are"} waiting for moderation.`
      : "Approve examples, then load them into the model.";
  }
...
function loadApprovedIntoModel() {
  ...
  const loaded = globalThis.OutreachTinyModel?.loadAudienceCorpus(examples);
  if (!loaded) return;
  const originalLabel = useApprovedDataset.textContent;
  useApprovedDataset.textContent = `Loaded ${examples.length} approved example${examples.length === 1 ? "" : "s"}`;
  window.setTimeout(() => { useApprovedDataset.textContent = originalLabel; }, 1800);
}
