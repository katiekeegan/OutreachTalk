/*
 * QR matrix code derived from qrcode-terminal 0.12.0 and Kazuhiko Arase's
 * QR Code Generator. See THIRD_PARTY_NOTICES.md for licenses and attribution.
 */
(function (global) {
  "use strict";
  const modules = {
...
module.exports = QRCode;
  ...
  function localRequire(id) {
    ...
    if (!factory) throw new Error(`QR module not found: ${normalized}`);
  ...
  const ErrorLevel = localRequire('./QRErrorCorrectLevel');
  ...
  function svg(text, options = {}) {
    const cellSize = Number(options.cellSize) || 4;
    const marginCells = Number.isFinite(options.marginCells) ? Number(options.marginCells) : 4;
    const code = new QRCode(0, ErrorLevel.M);
    ...
    for (let row = 0; row < count; row += 1) {
      for (let col = 0; col < count; col += 1) {
        if (code.isDark(row, col)) {
          path += `M${margin + col * cellSize},${margin + row * cellSize}h${cellSize}v${cellSize}h-${cellSize}z`;
        }
      }
    }
    const label = escapeXml(options.label || `QR code for ${text}`);
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" role="img" aria-label="${label}"><rect width="100%" height="100%" fill="#fff"/><path d="${path}" fill="#000"/></svg>`;
  }

  function svgDataUrl(text, options = {}) {
    return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg(text, options))}`;
  }

  global.OutreachQRCode = Object.freeze({ svg, svgDataUrl });
})(globalThis);
